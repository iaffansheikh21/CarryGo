...
const Categories: React.FC = () => {
  ...
  const getIconBgColorClass = () =>
    pathname === "/home4" ? "bg-[#FF0000]" : "bg-red-600";
  const getTextColorClass = () =>
    pathname === "/home4" ? "text-black" : "text-white";

  return (
    <div className="py-12 px-4 md:px-12 lg:px-24 bg-[#F2F2F2]">
      <h2 className="text-2xl font-normal text-left mb-8 text-black">
        Categories
      </h2>
      <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-4 gap-y-10 gap-x-6">
        {categories.map((category, index) => (
          ...
          >
            <div
              className={`rounded-full h-20 w-20 flex items-center justify-center mb-4 sm:mb-0 sm:mr-4 ${getIconBgColorClass()}`}
            >
